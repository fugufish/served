...
describe Served::HTTPClient do
  ...
  describe '#get' do
    it 'calls the endpoint with the correct query and headers' do
      expect(HTTParty).to receive(:get).with('http://host/test',
                                             query:   { q: 1 },
                                             headers: { 'Content-Type' => 'application/json' }).and_return(true)
  ...
  describe '#post' do
    it 'calls the endpoint with the correct query and headers' do
      expect(HTTParty).to receive(:post).with('http://host/test',
